function test_lloydmax_ord_vs_scale(quantRate)
...
for ii = M_LOOP_STARTVAL:length(M)
  for jj = REPREP_LOOP_STARTVAL:nRepReps
    ...
    parfor k = repStart:repStop
      ...
      % Known K - direct estimate of noise var. as epsilon
      %epsilon1=sqrt((1-alpha)*K(ii));
      %epsilon2=sqrt(alpha*(1-alpha)*K(ii));
